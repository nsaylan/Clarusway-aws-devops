...
def main_get():
    return render_template('index.html', developer_name = 'Necip', not_valid = False)

@app.route('/', methods = ['POST'])
def main_post():
    alpha = request.form['number']
    if not alpha.isdecimal():
        return render_template('index.html', developer_name = 'Necip', not_valid = True)
    if not (0<int(alpha)):
        return render_template('index.html', developer_name = 'Necip', not_valid = True)
    return render_template('result.html', developer_name = 'Necip', milliseconds = int(alpha), result = convert(int(alpha)))
