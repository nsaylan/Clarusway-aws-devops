...
$ vim hosts
[webservers]
node1 ansible_host=<node1_private ip> ansible_user=ec2-user
node2 ansible_host=<node2_private ip> ansible_user=ec2-user
...
- Write the code (in code node) below to check testfile was sent.
```bash
...
```bash
ansible -i inventory.ini -b -m yum -a "name=httpd state=present" node1 
ansible -i inventory.ini -b -m yum -a "name=httpd state=absent" node1 
```
